...
type LocalConfig(config: ConfigFile) =
    ...
    member val EnablePossessionProtocol =
        let description =
            "If enabled, only one protocol mimic can be active at a time."
                + " Eligible deaths are queued (FIFO), mimic kills do not convert victims, and queue progression follows the Possession Protocol rules."
        config.Bind<bool>(
            section,
            "Enable Possession Protocol",
            true,
            description
        )
...
let private toSyncedConfig () =
    {   reviveChance = localConfig.Value.ReviveChance.Value
        ...
        enablePossessionProtocol = localConfig.Value.EnablePossessionProtocol.Value
    }
